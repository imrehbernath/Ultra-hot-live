...
const App: React.FC = () => {
  ...
  const loadTrends = useCallback(async (isManual: boolean = false) => {
    ...
    try {
      ...
      if (data.trends && data.trends.length > 0) {
        ...
        setError("No data received. Retrying signal...");
      }
    } catch (err: any) {
      if (err.message === "API_KEY_MISSING") {
        setError("API Key required. System offline.");
      } else {
        setError("Signal interference. Re-acquiring feed...");
      }
      setIsActive(false);
    } finally {
      setLoading(false);
  ...
  }, [isActive, loadTrends, hasPaidKey, activeTab]);
  ...
  const handleActivateSystem = async () => {
    ...
    setIsActive(true);
    if (activeTab === 'radar') loadTrends(true);
  };

  const exportToCSV = () => {
    if (trends.length === 0) {
      alert("Feed offline. Activate system to generate keywords.");
      return;
    }
    const csvContent = trends
      .map(t => t.topic.trim().toUpperCase())
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-orange-500/30 overflow-x-hidden relative">
            ...
            <div key={i} className="flex items-center">
              {trends.length > 0 ? trends.map((t) => (
                ...
                <span className="mx-8 text-[10px] font-black uppercase tracking-widest text-black flex items-center gap-2">
                  SYSTEM STATUS: {hasPaidKey ? 'READY' : 'OFFLINE'} — AWAITING UPLINK COMMAND
                </span>
              )}
            </div>
          ))}
          ...
          <div className="flex items-center gap-8">
            ...
            <nav className="hidden md:flex items-center gap-1 bg-white/5 p-1 rounded-full">
              <button 
                ...
                className={`px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'radar' ? 'bg-orange-600 text-black' : 'text-slate-500 hover:text-white'}`}
              >
                Global Radar
              </button>
              <button 
                onClick={() => setActiveTab('scanner')}
                className={`px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'scanner' ? 'bg-orange-600 text-black' : 'text-slate-500 hover:text-white'}`}
              >
                Screenshot Scan
              </button>
            </nav>
          ...
          <div className="flex gap-2">
            {activeTab === 'radar' && (
              <button 
                ...
                className={`p-2 border rounded-full transition-all ${trends.length > 0 ? 'border-orange-500/50 hover:bg-orange-500/10 text-orange-500' : 'border-white/5 text-slate-800 opacity-50 cursor-not-allowed'}`}
                title="Download CSV Keywords"
              >
            ...
            {!isActive ? (
              <button onClick={handleActivateSystem} className="bg-orange-600 text-black px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all">
                {hasPaidKey ? 'Launch Feed' : 'Set API Key'}
              </button>
            ) : (
      ...
      <main className="max-w-[1400px] mx-auto px-6 py-12">
        {error && (
          <div className="mb-8 p-4 bg-red-900/20 border border-red-900/50 rounded-2xl text-red-500 text-xs font-bold text-center animate-pulse">
            {error}
          </div>
        )}
        ...
        {activeTab === 'radar' ? (
              ...
              {!isActive && trends.length === 0 ? (
                <div className="text-center py-20 space-y-8">
                  <h2 className="text-7xl lg:text-9xl font-black tracking-tighter leading-none italic uppercase">
                    Global <br/> <span className="text-orange-600 outline-text">Intelligence.</span>
                  </h2>
                  <p className="text-slate-500 font-bold tracking-[0.3em] uppercase text-[10px]">Real-time keyword extraction engine</p>
                  <button onClick={handleActivateSystem} className="bg-white text-black px-12 py-5 rounded-full font-black text-sm uppercase tracking-widest hover:bg-orange-500 transition-all shadow-[0_20px_50px_rgba(255,255,255,0.05)]">
                ...
                globalSummary && (
                  <div className="bg-white/[0.03] border border-white/10 rounded-[2rem] p-8 md:p-12 mb-12 relative overflow-hidden group">
                    ...
                    <h2 className="text-xs font-black text-slate-500 uppercase tracking-[0.3em] mb-4">World Update</h2>
            ...
            {sources.length > 0 && (
              <div className="mt-20 py-10 border-t border-white/5">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Grounding Verification</h3>
                  <span className="text-[8px] font-black text-orange-500/50 uppercase tracking-widest">Authored via Google Search</span>
                </div>
          ...
          <div className="max-w-6xl mx-auto">
            <div className="mb-12 text-center md:text-left">
              <h2 className="text-5xl lg:text-7xl font-black italic tracking-tighter uppercase mb-4">
                Screenshot <span className="text-orange-500">Scanner</span>
              </h2>
              <p className="text-slate-500 font-bold uppercase tracking-widest text-xs">Analyze pump.fun & DEX screenshots for high-velocity alpha</p>
            </div>
            <ScreenshotScanner />
          </div>
        )}
      ...
      <footer className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-full max-w-lg px-4">
        <div className="bg-slate-900/90 backdrop-blur-2xl border border-white/10 rounded-full px-8 py-4 flex items-center justify-between shadow-2xl">
          <div className="flex flex-col">
            <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Mode</span>
            <span className="text-xs font-mono font-bold text-orange-500 uppercase">{activeTab}</span>
          </div>
          <div className="flex flex-col items-end">
            <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Update Cycle</span>
            <span className="text-xs font-mono font-bold">{isActive && activeTab === 'radar' ? `${nextUpdateIn}s` : 'MANUAL'}</span>
          </div>
        </div>
      </footer>
    </div>
